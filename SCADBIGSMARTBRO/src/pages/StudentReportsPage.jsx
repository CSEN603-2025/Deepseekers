...
function StudentReportsPage() {
  ...
  const loadUserData = () => {
    ...
    const evaluationsByInternship = parsedEvaluations.filter(evaluation => evaluation.studentId === userId);
    const reportsByInternship = parsedReports.filter(report => report.studentId === userId);
    ...
    evaluationsByInternship.forEach(evaluation => {
      const exists = combinedReports.some(report => report.internshipId === evaluation.internshipId);
      if (!exists) {
        combinedReports.push({
  ...
  const handleFinalSubmission = () => {
    ...
    setTimeout(() => {
      ...
      const reportIndex = reports.findIndex(report => report.id === selectedItem.reportId);
      if (reportIndex >= 0) {
        reports[reportIndex] = {
          ...reports[reportIndex],
          isSubmitted: true,
    ...
    <>
      <Container className="student-reports-container">
        ...
        <p className="page-description">
          View and submit your complete internship reports with company evaluations.
        </p>
          ...
          {finalizedReports.length > 0 ? (
            finalizedReports.map((report) => (
              <Col md={6} lg={4} key={report.id} className="mb-4">
                <Card className={`finalized-report-card ${report.isFinalized ? 'complete' : 'incomplete'}`}>
                  <Card.Header>
                    <div>
                    ...
                    <div className="status-badges">
                      {report.recommend && (
                        <Badge bg="success" className="recommend-badge">Recommended</Badge>
                      )}
                      {report.isSubmitted ? (
                        <Badge bg="info" className="status-badge">Submitted</Badge>
                      ) : report.isFinalized ? (
                        <Badge bg="warning" className="status-badge">Ready to Submit</Badge>
                      ) : (
                        <Badge bg="secondary" className="status-badge">Incomplete</Badge>
                      )}
                    </div>
                  </Card.Header>
                  <Card.Body>
                    ...
                    <div className="report-components">
                      <div className="component">
                        ...
                        <span className={report.reportBody ? 'completed' : 'missing'}>
                          {report.reportBody ? 'Completed' : 'Missing'}
                        </span>
                      </div>
                      <div className="component">
                        <span className="component-label">Evaluation:</span>
                        <span className={report.hasEvaluation ? 'completed' : 'missing'}>
            ...
            <Col xs={12}>
              <div className="empty-state">
                ...
                <p>You haven't created any evaluations or reports yet. Complete your internship and create reports (In Internships tab) to see them here.</p>
              </div>
            </Col>
          )}
        </Row>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>
            {selectedItem?.reportTitle || `Report for ${selectedItem?.companyName}`}
          </Modal.Title>
        </Modal.Header>
        <Modal.Body>
          {selectedItem && (
            <div className="finalized-report-details">
              ...
              <div className="detail-header">
                <span className="detail-label">Last Updated:</span> {formatDate(selectedItem.date)}
              </div>
              ...
              {selectedItem.helpfulCourses && selectedItem.helpfulCourses.length > 0 && (
                <div className="detail-content mt-4">
                  ...
                  <ul className="helpful-courses-list">
                    {getCourseNames(selectedItem.helpfulCourses).map((course, index) => (
              ...
              {!selectedItem.isFinalized && (
                <div className="incomplete-warning mt-4">
                  <p>This report is incomplete. Please make sure you have both a report and an evaluation to finalize it.</p>
                </div>
              )}
            </div>
          )}
      ...
      >
        ...
        <Modal.Body>
          {submissionMessage ? (
            ...
            <>
              <p>You are about to submit your finalized report for <strong>{selectedItem?.companyName}</strong>.</p>
              <p>Once submitted, it cannot be modified. Are you sure you want to proceed?</p>
            </>
          )}
        </Modal.Body>
        <Modal.Footer>
          {!submissionMessage && (
            <>
              ...
              <Button variant="primary" onClick={handleFinalSubmission} disabled={isSubmitting}>
